...
However, a whole class of models have now been developed that predict similar phenomena as the original theories where either one or two light, neutral bosons
\footnote{While the original majoron was postulated as a Goldstone boson, depending on the model, the majoron may or may not be a Goldstone boson.}
are emitted during a neutrinoless double-beta decay.
...
\hline
``bulk" & \zeronubbonechi & no              & 0    & 2              \\ 
\end{tabular}
\caption[The majoron models and their associated decay modes.]
{The majoron models and their associated decay modes.
For \zeronubbtwochi, each majoron has an associated lepton number of $-1$, which precisely cancels out the $\Delta L$ carried by the electrons in these \zeronubbtwochi~decays.}
...
\end{align}
where $E_{\textrm{sum}}$ is the summed energy of the electrons, $F(Z, E)$ is the Fermi function for a particular nucleus with atomic number Z, \color{red} define Fermi function \color{black}, $E_i$ and $p_i$ are the energy and momentum of each particle, respectively, and $n$ is the spectral index.
...
\begin{align}
    ...
    y &= \alpha Z W / p
\end{align}
where $\rho$ is the nuclear radius, \color{red} p is something, maybe momentum?, got from this article: https://journals.aps.org/pr/pdf/10.1103/PhysRev.150.846, but check w/ code \color{black}.
...
With this, the background at $Q_{\beta\beta}$ for the BEGe detectors is low enough at $(0.07^{+1.1}_{-0.5}) \cdot 10^{-3} /(\textrm{keV} \cdot \textrm{kg} \cdot \textrm{yr})$ allowing for the experiment to be considered background-free at the design exposure of $100 \textrm{kg \cdot yr}$.
...
The detector uses the Xe as a liquid scintillator acting as both the source and detector of \zeronubb. Xe benefits from self-shielding due to the attenuation length of gammas in xenon which allows for reduced levels of background in the innermost volume.
...
However, the energy resolution of the KamLAND-Zen is significantly worse than the $\mathcal{O}(\textrm{keV})$ resolution in these experiments.
...
NEMO-3 also searched for \zeronubb~in $^{100}$Mo, $^{82}$Se, $^{48}$Ca, and $^{150}$Nd \cite{Bongrand:2011ei}\cite{::2016dpe}\cite{Arnold:2016ezh}.
