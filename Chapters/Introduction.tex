...
The Standard Model is one of the most powerful theories in physics, and together with $\Lambda$CDM  in cosmology, has been able to correctly predict phenomena on both subatomic and cosmological scales \cite{Aghanim:2018eyx}.
...
With the mass of neutrinos shown to be non-zero, the Standard Model can be extended to include the masses for the neutrinos, but this opens up another question for how exactly the neutrino mass comes into the Standard Model Lagrangian.
For the other fundamental fermionic particles, the corresponding antiparticle will have the opposite charge\footnote{for example an electron with charge $\textrm{e}^-$ is paired with the positron with charge $\textrm{e}^+$}.
...
While these effects satisfy the other two Sakharov conditions, there is not enough of an effect to explain the measured asymmetry,
\begin{align}
